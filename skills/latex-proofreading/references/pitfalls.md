...
## 作者会从旧副本编辑，把已改好的地方改回去

**这是实测过的流程陷阱，不是理论风险。** 一批修复（表头大小写、DOI 大小写、
highlight 字符数、符号记号等 5 处）在两轮之间**被回退过两次**。

常见成因：作者从一个较早的副本复制段落、编辑器里有未保存的旧缓冲、
或多人各存一份再合并。

**症状**：新版本里出现"上一轮明明改过、现在又变回原样"的地方。

**对策**：

1. **留着上一轮的修复清单**。每次拿到新版，先逐条 `grep -F` 确认修复是否还在——
   成本极低，能立刻发现回退。
2. 回退项在报告里**单独列一组**，并注明"这几处已经丢过 N 次"。
   **重复回退通常是流程问题，不是笔误**，值得请作者查一下编辑流程。
3. 不要因为"以前改过"就无条件改回去——作者可能是**有意**改回的。
   把判据重新讲一遍，让作者定夺。
...
同一篇稿件里**指标同名的多张表**是常态——仿真一张、实测一张，不同工况再各一张，
列名与指标完全一样。凭印象取数去对正文，会报出假错。

**实测代价**：曾用仿真消融表的数据去对正文里的实测结果，报了一条"数字矛盾"，
而其修正方向与实测表**恰好相反**。**作者若照改，就把正确的数字改错了。**

**流程**：从正文那句话里的 `\ref{}` 读出**表号**（是标签真正指向的那张，
不是"看起来像"的那张）→ `consistency.py --tables` 导出那一张 → 对不上时先怀疑自己拿错表。

---

## 已经交付的建议被推翻：必须显式撤回

报告发出之后才发现某条建议是错的，**这时作者可能已经照着改了**。
静默丢弃 = 在稿子里留下一个事实错误，而且无人知晓。

**撤回三条**：

1. **单独成条** —— 编号、逐字原文、错在哪，不要混进"其他修改"里
2. **给出回退** —— "如果你已经改了，请改回：`<原文>`"
3. **说清证据** —— 具体到某张表、某个数；不要写"重新考虑后觉得不妥"

**常见成因**：拿 A 维度的数据去支持 B 维度的措辞。实测里出现过把
"定位精度"与"参数精度"混为一谈——建议改写为"参数估计基本不变"，
而表里那个参数的变化幅度接近一个数量级。
...
（例如给一篇 0 个 em dash 的论文加 em dash）。
