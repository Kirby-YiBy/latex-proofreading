...
**已经交付的建议被推翻时，必须显式撤回 —— 不能静默丢弃。**

报告发出之后才发现某条建议是错的，**这时作者可能已经照着改了**。
你只是在下一轮里不再提它，稿子里就会留下一个事实错误，而且没人知道。

撤回要满足三条：

1. **单独成条**：编号、逐字原文、错在哪，写清楚（不要混进"其他修改"里）
2. **给出回退**："如果你已经改了，请改回：<原文>"
3. **说清证据**：具体到某张表、某个数；不要写"重新考虑后觉得不妥"

**这类错误多半有同一个成因**：拿 A 维度的数据去支持 B 维度的措辞。
实测里出现过把"定位精度"与"参数精度"混为一谈 —— 建议改写为"参数估计基本不变"，
而表里那个参数的变化幅度接近一个数量级。
...
- 我准备报一条"数字对不上" → **先确认正文指的是哪张表（同篇可能有指标同名的多张表）**
- 我发出去过的某条建议后来发现是错的 → **单独成条撤回，并写明"如果已改请改回"；不要静默丢弃**
